...
def plot_heatmap(ax, data, title, vmin, vmax, xticklabels, yticklabels, reverse=False):
    ...
    im = ax.imshow(matrix, cmap='coolwarm', aspect='equal', vmin=vmin, vmax=vmax)
...
def visualize_HeatMap(total_task_info, xticklabels, yticklabels):
    ...
    gs = gridspec.GridSpec(1, 3, height_ratios=[1], width_ratios=[1, 1, 0.06], wspace=0.5)
    ...
    plt.subplots_adjust(left=0.1, right=0.93, top=0.85, bottom=0.15, wspace=0)
...
def visualize_single_HeatMap(total_task_info, xticklabels, yticklabels):
    ...
    fig, (ax, cax) = plt.subplots(
        ...
        ncols=2, 
        figsize=(10, 5),
        gridspec_kw={"width_ratios": [1, 0.03], "wspace": -0.3}
    )

    en_data = np.array(info[0]["record"])
    vmin = min(np.min(data) for data in en_data)
    vmax = max(np.max(data) for data in en_data)
    print(f"\nvmin: {vmin}")
    print(f"\nvmax: {vmax}")
