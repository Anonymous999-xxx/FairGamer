...
def plot_heatmap(ax, data, title, vmin, vmax, xticklabels, yticklabels, reverse=False):
    ...
    im = ax.imshow(matrix, cmap='coolwarm', aspect='equal', vmin=vmin, vmax=vmax)  # 设置 aspect='equal'
...
def visualize_HeatMap(total_task_info, xticklabels, yticklabels):
    ...
    """
    info = total_task_info
    # game_name = info[0]['game_name'][0]

    # 创建一个大图，使用 GridSpec 控制布局
    fig = plt.figure(figsize=(14, 7))  # 调整画布大小
    gs = gridspec.GridSpec(1, 3, height_ratios=[1], width_ratios=[1, 1, 0.06], wspace=0.5)  # 三个子图：热力图1、热力图2、颜色条
    # fig.suptitle(f'Discount (Unreal) - DeepSeek-V3', fontsize=16)
    ...
    for i, task in enumerate(info):
        ...
        sorted_matrix, sorted_xticklabels, sorted_yticklabels = sort_matrix_and_labels(
            record, xticklabels[xy_stick_lang], yticklabels[xy_stick_lang])
        
        # 在对应的子图上绘制热力图
        if i == 0:
            im = plot_heatmap(ax1, sorted_matrix, title[i], vmin, vmax, sorted_xticklabels, sorted_yticklabels)
        else:
            im = plot_heatmap(ax2, sorted_matrix, title[i], vmin, vmax, sorted_xticklabels, sorted_yticklabels)
    ...
    # 调整整体布局
    plt.subplots_adjust(left=0.1, right=0.93, top=0.85, bottom=0.15, wspace=0)  # 调整左右边距，使整体布局居中

    # 保存图片，并设置分辨率为 500 DPI
    plt.savefig("./img/virtual/HeatMap-DeepSeekV3.pdf", dpi=500)

    plt.show()

def visualize_single_HeatMap(total_task_info, xticklabels, yticklabels):
    """
    显示单个热力图(仅En)
    :param total_task_info: 任务信息列表
    :param xticklabels: 多语言(En & Zh)的横坐标标签列表
    :param yticklabels: 多语言(En & Zh)的纵坐标标签列表
    """
    info = total_task_info
    ...
    fig, (ax, cax) = plt.subplots(
        ...
        ncols=2, 
        figsize=(10, 5),  # 调整画布大小
        gridspec_kw={"width_ratios": [1, 0.03], "wspace": -0.3}  # 主图和颜色条的比例
    )
    '''
    # normalize:
    print("len(info)={}".format(len(info)))
    for i in range(len(info)):
        record = np.array(info[i]["record"]) / 100
        info[i]["record"] = record.tolist()
    '''
    # 找到所有数据的最小值和最大值，用于统一颜色映射范围
    en_data = np.array(info[0]["record"])
    vmin = min(np.min(data) for data in en_data)  # 全局最小值
    vmax = max(np.max(data) for data in en_data)  # 全局最大值
    print(f"\nvmin: {vmin}")
    print(f"\nvmax: {vmax}")
    ...
    # 绘制热力图
    im = plot_heatmap(ax, sorted_matrix, title, vmin, vmax, sorted_xticklabels, sorted_yticklabels)
    
    # 添加颜色条
    cbar = fig.colorbar(im, cax=cax)
    # cbar.set_label('Average Value', fontsize=14)
    
    # 调整布局
    plt.tight_layout()

    # 保存图片
    plt.savefig("./img/virtual/Single_HeatMap-DeepSeekV3.pdf", dpi=500, bbox_inches='tight')
    plt.show()
...
def visualize_Decision_lstd(
    ...
    # 保存图片，并设置分辨率为 500 DPI
    # plt.savefig("./img/virtual/SNPC-virtual_D_lstd.pdf", dpi=500)
...
def visualize_D_cl(
    ...
    # 添加标题和标签
    # plt.title('Task 2 (Unreal): English - Chinese', fontsize=title_fontsize)
    plt.xlabel('Model', fontsize=label_fontsize)
    ...
    # 保存图片，并设置分辨率为 500 DPI
    plt.savefig("./img/virtual/SNPC-virtual_D_cl.pdf", dpi=500)
    plt.savefig("./img/virtual/SNPC-virtual_D_cl.png", dpi=500)

    # 显示图形
    plt.show()

    # 输出结果，以表格形式
    print("D_cl in SNPC-virtual (Mean):")
    print("------------------------------------------------------------------------------")
    print("| Model Name | Value |")
    for i in range(len(model_name)):
        print("| {} | {:.3f} |".format(
    ...
    print("D_cl in SNPC-virtual (Std):")
    print("------------------------------------------------------------------------------")
    print("| Model Name | Value |")
    for i in range(len(model_name)):
        print("| {} | {:.3f} |".format(
...
if __name__ == '__main__':
    ...
    game_race = game_race["virtual"]
    game_career = read_json('./data/career-SNPC.json')
    game_career = game_career["virtual"]
    ...
    for m in model_record_list:
        model_path = './record/SNPC_virtual_{}_raw-final.json'.format(m)
    ...
    # Plot 4:
    visualize_single_HeatMap(model_data_raw[3], game_career, game_race)